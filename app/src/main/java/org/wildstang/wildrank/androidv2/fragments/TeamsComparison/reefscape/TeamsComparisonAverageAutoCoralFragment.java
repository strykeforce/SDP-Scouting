        ...
        button.setOnClickListener(v -> {
            ...
            for (int i = 0; i < teams.size(); i++) {
                ...
                for (Document document : teamDocuments) {
                    ...
                    if (match != 0) {
                        levelOnePercentage += (float) (int) data.get("auto_level_one") / (float) match;
                        levelTwoPercentage += (float) (int) data.get("auto_level_two") / (float) match;
                        levelThreePercentage += (float) (int) data.get("auto_level_three") / (float) match;
                        levelFourPercentage += (float) (int) data.get("auto_level_four") / (float) match;
                    }
                }
